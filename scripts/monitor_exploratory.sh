#!/bin/sh
# Monitor the active schema-v6r1 exploratory live evaluation.
set -eu

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT"

LOG="$ROOT/live_exploratory.log"
PIDFILE="$ROOT/live_exploratory.pid"

echo "=== Exploratory log (last 12 lines) ==="
if [ -f "$LOG" ]; then
  tail -12 "$LOG"
else
...
states = [state for state in (state_for(path) for path in dict.fromkeys(candidate_paths)) if state]
if not states:
...
try:
    predictions = [
        json.loads(row[0])
        for row in connection.execute("SELECT data_json FROM forecasts ORDER BY sequence_no")
    ]
...
print(f"  schema/model/revision: v{state.get('schema_version')} / {state.get('model_version')} / {state.get('implementation_revision', 'pre-hardening')}")
print(f"  status:                {state.get('status')} ({state.get('stop_reason')})")
print(f"  observations:          {observations}")
print(f"  forecasts:             {len(predictions)} total, {len(pending)} pending")
print(f"  resolved:              {len(resolved)} total, {len(eligible)} eligible, {len(excluded)} excluded")
